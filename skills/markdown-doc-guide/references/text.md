...
（5）**Markdown 链接路径不得做任何修改。** `[文本](路径)` 中 `()` 内的路径字符串必须原样保留，禁止对其进行任何格式化处理，包括：插入空格、替换标点符号的全半角形式等。

```
错误：[JSON 序列化](工具/IO/JSON 序列化.md)   ← 路径中插入了空格
正确：[JSON 序列化](工具/IO/JSON序列化.md)

错误：[Java 序列化](Java_IO_序列化。md)        ← 路径中的 . 被替换为 。
正确：[Java 序列化](Java_IO_序列化.md)
```
